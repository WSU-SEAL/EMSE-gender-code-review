...
######################################################### RQ3
RQ3_latex_table_code =""

dynamic_vars = c( 'logTotalCommit','tenure' )

for (project in names(projects)) {
  #project="android"
  p_name =projects[project]
  datafile =paste("../dataset/", project, "-dataset2.csv", sep="")
  print(datafile)
  dataset_review = read.csv(datafile, header = TRUE)
  dataset_review$logTotalCommit=log(dataset_review$totalCommit+1)
  dataset_review$logRevExp=log(dataset_review$revExp+1)
  dataset_review$avgReviewPerMonth=dataset_review$revExp/dataset_review$tenure
  
  
 # survived_vars = AutoSpearman(dataset = dataset_review, metrics = dynamic_vars,verbose = T)
#  print(survived_vars)
  
  formula_string ="avgReviewPerMonth ~ isGenderNeutral + Gender"
  
  
  formula_string_factored = "avgReviewPerMonth ~ NeutralFactor * GenderFactor "
...
fit_glm_review <- lm(as.formula(formula_string) , data=dataset_review,  x=T, y=T) 
  
  
  model_summary =summary(fit_glm_review)
  
  
  adj_R_square=round(model_summary$adj.r.squared,3)
  
  
  model_coefficients <- coef(fit_glm_review)
  
  gender_bias =round(model_coefficients["Gender"],2)
  gender_pvalue =round(model_summary$coefficients["Gender", "Pr(>|t|)"],4)
  
 
  
  if (project=="libreoffice" |project =="qt" | project =="wikimedia" |project=="whamcloud")
  {
    RQ3_latex_table_code =paste(RQ3_latex_table_code, p_name, "&", 
                                adj_R_square, "&",
                                get_OR_code(gender_bias), "",
    ...
    dataset_review$GenderFactor <-factor(dataset_review$Gender, labels=c("M","F"))
    dataset_review$NeutralFactor <-factor(dataset_review$isGenderNeutral,  labels=c("N","Y"))
    
    gender_neutral_bias = round(model_coefficients["isGenderNeutral"],2)
    gender_neutral_pvalue =round(model_summary$coefficients["isGenderNeutral", "Pr(>|t|)"],4)
    
    print (formula_string_factored)
    
    fit_glm_review2 <- lm(as.formula(formula_string_factored) , data=dataset_review,  x=T, y=T)
    
    model_coefficients2 <- coef(fit_glm_review2)
    model_summary2 =summary(fit_glm_review2)
    
    adj_R_square2=round(model_summary2$adj.r.squared,3)
    
    print(paste("First", adj_R_square, " Second: ", adj_R_square2))
    
    WomanGendered =round(model_coefficients2["GenderFactorF"],2)
    wg_pvalue =round(model_summary2$coefficients["GenderFactorF", "Pr(>|t|)"],4)
    
    ManNeutral =round(model_coefficients2["NeutralFactorY"],2)
    mn_pvalue =round(model_summary2$coefficients["NeutralFactorY", "Pr(>|t|)"],4)
    
    WomanNeutral =round(model_coefficients2["NeutralFactorY:GenderFactorF"],2)
    wn_pvalue =round(model_summary2$coefficients["NeutralFactorY:GenderFactorF", "Pr(>|t|)"],4)
    
    RQ3_latex_table_code =paste(RQ3_latex_table_code, p_name, " & ", 
                                adj_R_square, " & ",
                                get_OR_code(gender_bias), "",
                                get_p_code(gender_pvalue), " & ",
                                get_GN_code(gender_neutral_bias), "",
