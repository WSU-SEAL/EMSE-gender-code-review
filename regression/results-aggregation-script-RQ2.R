...
for (project in names(projects)) {
  ...
  formula_string ="logReviewInterval ~ isBugFix +  isGenderNeutral + Gender"
  formula_string_factored = "logReviewInterval ~ isBugFix  +NeutralFactor * GenderFactor "
  ...
  fit_glm_interval <- lm(as.formula(formula_string) , data=dataset,  x=T, y=T  )
  
  #data_dict <- datadist(dataset)
  #options(datadist="data_dict")
  
  model_summary =summary(fit_glm_interval)
  
  
  adj_R_square=round(model_summary$adj.r.squared,3)
  
  
  model_coefficients <- coef(fit_glm_interval)
  
  gender_odds_ratio =round(exp(model_coefficients["Gender"]),2)
  ...
  {
    RQ2_latex_table_code =paste(RQ2_latex_table_code, p_name, "&", 
                                adj_R_square, "&",
    ...
    fit_glm_interval2 <- lm(as.formula(formula_string_factored) , data=dataset,  x=T, y=T 
                              )
    
    model_coefficients2 <- coef(fit_glm_interval2)
    model_summary2 =summary(fit_glm_interval2)
